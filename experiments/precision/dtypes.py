...
class bfloat16(dtype):
        ...
        for i in range(len(result)):
            ...
            for j in range(len(kernel)):
                acc = bf16_round(
                    acc +
                    bf16_round(data[i + j]) *
                    bf16_round(kernel[len(kernel) - 1 - j])
                )
            result[i] = acc
    ...
    def _add(data1: np.ndarray, data2: np.ndarray, result: np.ndarray):
        for i in range(len(result)):
            result[i] = bf16_round(
                bf16_round(data1[i]) + bf16_round(data2[i])
            )
...
class tfloat32(dtype):
        ...
        for i in range(len(result)):
            ...
            for j in range(len(kernel)):
                acc = tf32_round(
                    acc +
                    tf32_round(data[i + j]) *
                    tf32_round(kernel[len(kernel) - 1 - j])
                )
            result[i] = acc
    ...
    def _add(data1: np.ndarray, data2: np.ndarray, result: np.ndarray):
        for i in range(len(result)):
            result[i] = tf32_round(
                tf32_round(data1[i]) + tf32_round(data2[i])
            )


class mixed_bf16xf32(dtype):
    @staticmethod
    def conv(data: np.ndarray, kernel: np.ndarray) -> np.ndarray:
        data = np.array(data, dtype=np.float32)
        kernel = np.array(kernel, dtype=np.float32)
