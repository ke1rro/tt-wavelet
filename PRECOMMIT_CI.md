...
2. Шукає артефакт `tt-metal-v3-<hash>` у успішних запусках `full.yaml`.
...
- публікує артефакт з імʼям `tt-metal-v3-<submodule_sha>`.
3. Job `full`:
...
3. Переконайся, що з’явився артефакт `tt-metal-v3-<hash>`.
...
2. Full CI автоматично збере новий артефакт `tt-metal-v3-<new_hash>`.
