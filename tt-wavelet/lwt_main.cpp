...
// ---------------------------------------------------------------------------
// Lifting scheme definition
// ---------------------------------------------------------------------------
// Built from the step helpers in lifting_step.hpp and make_lifting_scheme().
// Mirrors the bior2.2 JSON (tap_size=6, delay_even=1, delay_odd=2).
//
// Predict steps read the even sub-band and update the odd sub-band.
// Update  steps read the odd  sub-band and update the even sub-band.
...
static void print_signal(const std::string& label,
...
static void run_step(
    ...
    LwtProgram prog = create_lwt_step_program(
        kernel_root, core,
        *in_buf->get_backing_buffer(),
        *out_buf->get_backing_buffer(),
        cfg);
...
int main() {
    ...
    SignalBuffer input_desc{
        .dram_address = 0, .length = kSignalLength,
        .stick_width = 32, .element_size_bytes = sizeof(float)};
    ...
    kScheme.for_each_step([&](const auto& step) {
        ...
        if constexpr (is_predict_step<Step> && !is_scale_step<Step>) {
            // Predict: reads even, writes odd
            const uint32_t K = static_cast<uint32_t>(step.n_coeffs);
            const std::string label = "step " + std::to_string(step_idx) +
                " predict (shift=" + std::to_string(step.shift) +
                ", K=" + std::to_string(K) + ") odd:";
            run_step(mesh_device.get(), cq, kernel_root, kCore,
                     cur_even_desc, even_buf,
                     cur_odd_desc,  odd_buf,
                     /*split_phase=*/0, step.shift, K, label);

        } else if constexpr (is_update_step<Step> && !is_scale_step<Step>) {
            // Update: reads odd, writes even
            const uint32_t K = static_cast<uint32_t>(step.n_coeffs);
            const std::string label = "step " + std::to_string(step_idx) +
                " update (shift=" + std::to_string(step.shift) +
                ", K=" + std::to_string(K) + ") even:";
            run_step(mesh_device.get(), cq, kernel_root, kCore,
                     cur_odd_desc,  odd_buf,
                     cur_even_desc, even_buf,
                     /*split_phase=*/1, step.shift, K, label);

        } else {
            std::cout << "step " << step_idx << " ("
                      << static_cast<int>(Step::type) << ") — skipped\n";
        }
    });
