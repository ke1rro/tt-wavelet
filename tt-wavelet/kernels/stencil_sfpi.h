...
inline void calculate_stencil_faces(
    const uint32_t h_packed[K], const uint input1, const uint input2, const uint output, const uint rows = 16) {
    // Vertical rows
#pragma unroll 4
    for (uint offset = 0; offset < std::min(rows, static_cast<uint>(16)) * 2; offset += 8) {
