...
void kernel_main() {
    ...
    constexpr uint32_t splice_number = get_arg_val<uint32_t>(arg_base + 5);

    constexpr uint32_t even_addr = get_arg_val<uint32_t>(arg_base + 0);
    constexpr uint32_t even_length = get_arg_val<uint32_t>(arg_base + 1);
    constexpr uint32_t even_offset = get_arg_val<uint32_t>(arg_base + 6);

    constexpr uint32_t odd_addr = get_arg_val<uint32_t>(arg_base + 2);
    constexpr uint32_t odd_length = get_arg_val<uint32_t>(arg_base + 3);
    constexpr uint32_t odd_offset = get_arg_val<uint32_t>(arg_base + 7);
    ...
    const auto src = TensorAccessor(even_accessor_args, src_addr, stick_nbytes);
    const auto base = TensorAccessor(odd_accessor_args, base_addr, stick_nbytes);
