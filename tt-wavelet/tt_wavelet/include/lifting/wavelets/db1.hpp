#pragma once

#include "../scheme.hpp"

namespace ttwv {

struct db1_tag {};

inline constexpr auto db1_scheme = make_lifting_scheme(
    2,
    0,
    1,
    PredictStep<1>{ { 1.0f }, 0 },
    SwapStep{ {}, 0 },
    PredictStep<1>{ { -0.49999999999999994f }, 0 },
    ScaleEvenStep<1>{ { 0.70710678118654746f }, 0 },
    ScaleOddStep<1>{ { -1.4142135623730951f }, 0 }
);

template <>
struct scheme_traits<db1_tag> {
    using SchemeType = decltype(db1_scheme);
    static constexpr const char* name = "db1";
    static constexpr int id         = 32;
    static constexpr int tap_size   = 2;
    static constexpr int delay_even = 0;
    static constexpr int delay_odd  = 1;
    static constexpr int num_steps  = 5;
    static constexpr const auto& scheme = db1_scheme;
};

}
