...
Intermediate writer переносить кожен вирівняний 16-element block з output CB у local L1
workspace одним stateful 64-byte NoC write. Після group `noc_async_writes_flushed()` чекає,
доки всі payloads покинуть source CB, і лише тоді writer звільняє його pages. Один повний
NoC barrier наприкінці route гарантує завершення записів до того, як наступний lifting route
