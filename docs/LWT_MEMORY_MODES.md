...
Один group навмисно формує 1536 output elements:

```text
source input:  2 full tiles = 2048 scalar positions
base input:    1 full tile + 1 half-used tail tile
output:        1 full tile + 1 half-used tail tile
valid output:  1024 + 512 = 1536 elements
```
...
Compute kernel розгортає static scheme через C++ templates. Для одного predict/update group:
...
```text
workspace_elements = round_up(max_chunk_max_workspace, 32)
...
Intermediate writer копіює 16-element blocks прямими L1 stores. Final writer пише 64-byte
half-stick blocks у global DRAM interval `output_offset + local_index`. Ознака final DRAM є
явним route flag, а не наслідком `StepType`, тому fused predict/update також може бути final
writer. Після кожного DRAM output group виконується NoC write barrier, щоб обмежити кількість
outstanding writes і не звільнити CB page завчасно.
...
- offsets і lengths лишаються у logical elements, адреси — у physical 32-element sticks;
